...
offset = 50000;
segmentLength = 40000;
traces2 = myload('traces.bin', traceSize, offset, segmentLength, numberOfTraces);
figure(2);
plot(traces2(2,:));
...
for BYTE=byteStart:byteEnd
    ...
    for K = keyCandidateStart:keyCandidateStop                            
        ...
        % Two AES first steps
        Hypothesis(1:length(numberOfTraces),K+1)=bitxor(plaintext(1:length(numberOfTraces),BYTE),K);
        Hypothesis(1:length(numberOfTraces),K+1)=SBOX(Hypothesis(1:length(numberOfTraces),K+1)+1);
    ...
    solvedKey(1:length(ligne),BYTE) = ligne - 1;
